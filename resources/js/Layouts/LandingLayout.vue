...
<template>
  <div class="min-h-screen flex flex-col">
    ...
    <!-- Fixed Header -->
    <header class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-7 py-4 sm:py-5 transition-colors"
        :class="isLandingPage ? '' : 'bg-white shadow-sm border-b border-gray-100'">

      <!-- Logo -->
      <h1 class="font-heading text-lg sm:text-xl pointer-events-auto transition-colors" :class="isLandingPage ? 'text-white' : 'text-gray-800'">
        BEL-IS
      </h1>

      <!-- Desktop Navigation -->
      <nav class="hidden lg:block pointer-events-auto">
        <div class="inline-flex px-6 py-3 rounded-md space-x-4 font-paragraph text-sm lg:text-base transition-colors"
            :class="isLandingPage ? 'bg-white/20 backdrop-blur-sm text-black' : 'text-gray-700'">
