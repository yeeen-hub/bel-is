...
<div class="min-h-screen">
        <!-- SIDEBAR -->
        <button class="sm:hidden p-3" @click="showSidebar = true">
            <i class="fa fa-bars text-xl"></i>
        </button>

        <div v-if="showSidebar" class="fixed inset-0 bg-black bg-opacity-40 z-40 sm:hidden"
            @click="showSidebar = false">
        </div>


        <aside class="fixed top-0 left-0 h-screen w-64 p-6 bg-gray-100 flex flex-col
           transform -translate-x-full sm:translate-x-0
           transition-transform duration-300 z-50" :class="{ 'translate-x-0': showSidebar }">
            ...
            <nav class="flex flex-col space-y-3 text-gray-600 text-base">
                ...
                <div v-if="can('view_reports')" class="flex flex-col">
                    <button @click="showReports = !showReports"
                        ...
                        <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showReports }" fill="none"
    ...
    <main class="ml-0 sm:ml-64 p-4 sm:p-8 bg-gray-50">
            <slot />
        </main>
</div>
