<template>
  <section id="map" data-section="map" class="bg-gray-50 py-10">
    <div class="max-w-5xl mx-auto px-4">
      ...
      <!-- Section Header -->
      <div class="mb-6">
        <h2 class="text-2xl sm:text-3xl font-semibold text-gray-900">Plan Your Circuit</h2>
        <p class="text-gray-500 text-sm mt-1">Choose destinations within Brgy. Belis to build your own tour route.</p>
      </div>

      <!-- MAP WRAPPER -->
      <!-- MAP CONTAINER -->
<div class="relative h-[500px] rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
  ...
  <!-- FLOATING PANEL -->
  <div class="absolute top-3 right-3 w-[300px] sm:w-[330px] h-[calc(100%-1.5rem)] bg-white rounded-xl shadow-xl border border-gray-100 flex flex-col overflow-hidden z-20">
    <!-- ===================== -->
    ...
    <div v-show="!selectedStop" class="flex flex-col h-full">
      ...
      <div class="p-4 border-b border-gray-100 flex-shrink-0">
        <div class="text-sm font-semibold text-gray-600 leading-tight">
          A community-led half-day to full-day tourism circuit entirely within Barangay Belis, Buruanga, Aklan
        </div>
        ...
        <div class="mt-3">
          <div class="flex items-center gap-2 bg-gray-50 border rounded-lg px-3 py-2"
            ...
            <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 111 11a6 6 0 0116 0z"/> </svg>
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Search destinations…"
              class="flex-1 text-xs bg-transparent text-gray-700 placeholder-gray-400 min-w-0
        ...
        <div class="flex gap-2 mt-3">
          <button @click="openInGoogleMaps" :disabled="stops.length < 1" class="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white rounded-full px-3 py-1.5 text-xs font-medium transition-colors flex items-center justify-center gap-1.5" > <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4"/> </svg> Open in Maps </button>
          <button @click="clearAll" :disabled="stops.length === 0" class="bg-gray-100 hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed border border-gray-200 rounded-full px-3 py-1.5 text-xs text-gray-600 transition-colors" > Clear </button>
        </div>
      </div>
    ...
    >
      <div
        v-if="selectedStop"
        class="absolute inset-0 bg-white flex flex-col z-30"
      >
        ...
        <div class="p-5 flex-1 overflow-y-auto">
          ...
          <div class="flex gap-2 mt-5">
            <a
              :href="`https://maps.google.com/?q=${encodeURIComponent(selectedStop.name)}`"
              target="_blank"
              class="flex-1 bg-blue-600 text-white text-sm rounded-full py-2.5 text-center"
            >
              View on Maps
            </a>
            ...
            <button
              @click="selectedStop = null"
              class="px-4 py-2.5 bg-gray-100 rounded-full text-sm"
            >
              Close
            </button>
          </div>
...
const DESTINATIONS = [
  {
    ...
    photo: 'https://lh5.googleusercontent.com/p/AF1QipNMHs-dMkN17fJ4IFVtbVVRqgf49NLhGm40Z28=w600-h400-p-k-no',
    lat: 11.8801399,
    lng: 121.8832658,
  ...
  },
  {
    ...
    photo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/cb/Philippines_fishing_village.jpg/1280px-Philippines_fishing_village.jpg',
    lat: 11.8814373,
    lng: 121.8826092,
  ...
  },
  {
    ...
    photo: 'https://lh5.googleusercontent.com/p/AF1QipOsEpbNT6MgAkIzVrMbmOjQqLBhPX_V8N0fxVY=w600-h400-p-k-no',
    lat: 11.8662018,
    lng: 121.880917,
  ...
  },
  {
    ...
    photo: 'https://lh5.googleusercontent.com/p/AF1QipPqv7VqvQ9bNTZV7P3F1CfKZjgQe4VmQ3OoNmg=w600-h400-p-k-no',
    lat: 11.8682332,
    lng: 121.8798559,
