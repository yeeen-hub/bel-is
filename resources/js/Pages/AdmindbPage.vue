    ...
    <SidebarLayout>
        ...
        <div class="container mx-auto">
            <div class="bg-gray-100 p-4 rounded-lg flex items-center gap-4">
                ...
                <!-- Search bar — filters recent visitors table -->
                <div class="relative flex-1">
                    <input
                        v-model="search"
                        type="text"
                        placeholder="Search visitors by name, origin, or purpose..."
                        :class="[
                    ...
                    />
                    <svg class="absolute left-2.5 top-2.5 w-4 h-4 pointer-events-none"
                    ...
                    <div v-if="showNotifications"
                        ...
                        <div class="max-h-72 overflow-y-auto">
                            <div v-if="stats.pending_fees > 0"
                                ...
                                <svg class="w-4 h-4 text-yellow-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                                </svg>
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-gray-800">{{ stats.pending_fees }} unpaid environmental fee(s)</p>
                                    <p class="text-xs text-gray-500 mt-0.5">These registrations are incomplete. Please collect payment.</p>
                                    <Link :href="route('visitor-records')" @click="showNotifications = false"
                            ...
                            <div v-if="stats.pending_pre_reg > 0"
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-gray-800">{{ stats.pending_pre_reg }} online pre-registration(s) awaiting</p>
                                    <p class="text-xs text-gray-500 mt-0.5">Visitors pre-registered online. Look up their code at the registration desk.</p>
                                    <Link :href="route('registration')" @click="showNotifications = false"
        ...
        <div class="container mx-auto bg-gray-100 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 p-4 gap-4 mt-2">
                <div class="bg-gray-900 p-4 rounded-lg shadow-md">
                    ...
                    <p class="text-2xl font-bold text-white">{{ stats.total_tourists.toLocaleString() }}</p>
                    <p class="text-white text-xs mt-1">{{ stats.total_tourists_today }} tourist(s) today</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 p-4 gap-4">
                <div class="bg-white p-4 rounded-lg shadow-md">
                    ...
                    <p class="text-2xl font-bold text-gray-800">{{ stats.visitors_this_month.toLocaleString() }}</p>
                ...
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <span v-if="stats.revenue_is_estimated" class="text-xs bg-yellow-100 text-yellow-700 font-semibold px-1.5 py-0.5 rounded">est.</span>
                    </div>
                    <p class="text-2xl font-bold text-gray-800">₱{{ Number(stats.revenue_today).toLocaleString('en-PH', { minimumFractionDigits: 2 }) }}</p>
                    <p class="text-gray-500 text-xs mt-1">₱{{ Number(stats.revenue_this_month).toLocaleString('en-PH', { minimumFractionDigits: 2 }) }} this month</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <div class="flex items-center justify-between mb-1">
                    ...
                    <p class="text-2xl font-bold text-gray-800">₱{{ Number(stats.revenue_this_year).toLocaleString('en-PH', { minimumFractionDigits: 2 }) }}</p>
                    <p class="text-gray-500 text-xs mt-1">{{ stats.revenue_is_estimated ? 'Based on collected fee_status × ₱100' : 'Total environmental fees collected' }}</p>
                </div>
                <div class="p-4 rounded-lg shadow-md border-2 transition"
                    :class="stats.pending_fees > 0 ? 'bg-yellow-50 border-yellow-300' : 'bg-white border-transparent'">
                    <h2 class="font-medium text-sm" :class="stats.pending_fees > 0 ? 'text-yellow-800' : 'text-gray-800'">Pending Fees</h2>
                    <p class="text-2xl font-bold" :class="stats.pending_fees > 0 ? 'text-yellow-700' : 'text-gray-800'">{{ stats.pending_fees }}</p>
            ...
            <div v-if="stats.pending_pre_reg > 0" class="px-4 pb-2">
                <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-blue-800">{{ stats.pending_pre_reg }} pending online pre-registration(s)</p>
                            <p class="text-xs text-blue-600 mt-0.5">Visitors who filled the online form are waiting to be processed at the checkpoint.</p>
                        </div>
                    </div>
                    ...
                    <div v-if="visitorsPerDay.length">
                        ...
                        <div class="flex gap-2 mt-2">
                            <div v-for="day in visitorsPerDay" :key="'lbl-' + day.date"
                                class="flex-1 text-center text-xs text-gray-400">{{ day.day }}</div>
                        </div>
                    </div>
                    ...
                    <div v-if="visitorsPerMonth.length">
                        ...
                        <div class="flex gap-2 mt-2">
                            <div v-for="month in visitorsPerMonth" :key="'lbl-' + month.month"
                                class="flex-1 text-center text-xs text-gray-400">{{ month.month.split(' ')[0] }}</div>
                        </div>
                    </div>
                    <p v-else class="text-gray-400 text-sm text-center py-8">No data available</p>
                </div>
                    ...
                    <div v-if="topOrigins.length" class="space-y-3">
                        <div v-for="(origin, index) in topOrigins" :key="origin.origin">
                            ...
                            <div class="bg-gray-100 rounded-full h-1.5 w-full">
                                <div class="bg-gray-900 h-1.5 rounded-full" :style="{ width: progressWidth(origin.count, topOrigins) }"></div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow-md">
                    ...
                    <div v-if="purposeBreakdown.length" class="space-y-3">
                        <div v-for="item in purposeBreakdown" :key="item.purpose">
                            ...
                            <div class="bg-gray-100 rounded-full h-1.5 w-full">
                                <div class="bg-blue-500 h-1.5 rounded-full" :style="{ width: progressWidth(item.count, purposeBreakdown) }"></div>
                            </div>
                        </div>
                    </div>
                    <p v-else class="text-gray-400 text-sm text-center py-4">No data available</p>
            ...
            <div class="p-4 mt-2">
                <div class="w-full p-4 shadow-md rounded bg-white">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h1 class="text-lg font-semibold">Recent Visitors</h1>
                            <!-- Search result count -->
                            <p v-if="search" class="text-xs text-gray-400 mt-0.5">
                                {{ filteredVisitors.length }} result(s) for "{{ search }}"
                            </p>
                        </div>
                        ...
                        <tbody>
                            <tr v-if="filteredVisitors.length === 0">
                                <td colspan="6" class="p-8 text-center text-gray-400">
                                    {{ search ? `No visitors found matching "${search}"` : 'No tourist records yet.' }}
                                </td>
                            </tr>
                            <tr v-for="visitor in filteredVisitors" :key="visitor.id"
                                class="hover:bg-gray-50">
                                <td class="p-2 border-b font-medium text-gray-800">
                                    <!-- Highlight matching text -->
                                    <span v-html="highlight(visitor.name)"></span>
                                </td>
                                <td class="p-2 border-b text-gray-600">
                                    <span v-html="highlight(visitor.place_of_origin)"></span>
                                </td>
                                <td class="p-2 border-b text-gray-600">
                                    <span v-html="highlight(visitor.purpose)"></span>
                                </td>
                                ...
                                <td class="p-2 border-b">
                                    <div class="relative inline-block" :ref="el => setDropdownRef(el, visitor.id)">
                                        <button @click.stop="toggleFeeDropdown(visitor.id)"
                                            :class="{
                                                ...
                                                'bg-red-100 text-red-500':       visitor.fee_status === 'No Show',
                                            }"
...
const props = defineProps({
    stats: {
        type: Object,
        default: () => ({
            total_tourists: 0, total_tourists_today: 0, visitors_this_month: 0,
            pending_fees: 0, revenue_today: 0, revenue_this_month: 0,
            revenue_this_year: 0, total_spots: 0, total_resorts: 0, total_beaches: 0,
            revenue_is_estimated: false, pending_pre_reg: 0,
        }),
    },
...
// Client-side filter across name, place_of_origin, and purpose
const filteredVisitors = computed(() => {
    if (!search.value.trim()) return props.recentVisitors
    const q = search.value.trim().toLowerCase()
    return props.recentVisitors.filter(v =>
        (v.name           ?? '').toLowerCase().includes(q) ||
        (v.place_of_origin?? '').toLowerCase().includes(q) ||
...
const highlight = (text) => {
    if (!search.value.trim() || !text) return text
...
const dropdownRefs = {}
const setDropdownRef = (el, id) => { if (el) dropdownRefs[id] = el }
...
const handleClickOutside = (e) => {
    ...
    if (activeRef && !activeRef.contains(e.target)) activeFeeDropdown.value = null
    if (!e.target.closest('.relative')) showUser.value = false
}
